...
#
# # FREQUENCY DEFINITIONS
# Rabi_freq = 1  # Rabi frequency in MHz 62 real life estimate
# f_0 = 0  # set the reference
# f_res = 500  # Let's say
...
# #     amp = 8000*t_width/0.05
# #     N_T_sampling = 10000  # T sampling for these !!!!
# #     t_span = np.linspace(0, interaction_time + t_avr, N_T_sampling) * 0.001  # t in ms
# #     # t_span = np.arange(0,interaction_time+t_avr,dt_pref)
# #     rho_t = integrate.odeint(von_neumann_tuneable, rho_0, t_span, args=(Rabi_freq, f_res, f_0, t_start, t_separation, 0.05, interaction_time, amp, gamma))
# #     np.transpose(rho_t)
...
#     t_span = np.linspace(0, interaction_time + t_avr, N_T_sampling) * 0.001  # t in ms
#     # t_span = np.arange(0,interaction_time+t_avr,dt_pref)
#     rho_t = integrate.odeint(von_neumann_tuneable, rho_0, t_span, args=(Rabi_freq, f_res, f_0, t_start, t_separation, t_width, interaction_time, 8000, gamma))
#     np.transpose(rho_t)
#     Exited_t = NORM - rho_t[:, 0]
#     dt = t_span[0] - t_span[1]
#     N_avr = int(t_avr * 0.001 / dt)
